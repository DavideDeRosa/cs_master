...
%
% Il | indica più possibili forme.
%
% leaf e node indicano vengono aggiunti per discriminare le forme possibili.
% Nel nostro caso non sarebbe stato necessario, ma in casi dove la lunghezza
% delle tuple è uguale è un buon modo per differenziare più forme.
% Questo permette di avere codice più robusto.
...
insert(K, V, {leaf, K2, V2}) when K =< K2 -> 
    {node, {leaf, K, V}, K, {leaf, K2, V2}} ;
insert(K, V, {leaf, K2, V2}) -> {node, {leaf, K2, V2}, K2, {leaf, K, V}} ;
insert(K, V, {node, T1, K2, T2}) when K =< K2 -> 
    {node, insert(K, V, T1), K2, T2} ;
